...
type Student = User & { createdAt: string; lastLoginAt: string | null; deviceBoundAt: string | null };
...
function AdminDashboard({ user, onLogout }: { user: User; onLogout: () => void }) {
  ...
  const resetDevice = async (student: Student) => { if (!window.confirm(`Reset the registered device for ${student.fullName}? They will need to log in again on the new device.`)) return; setBusyId(student.id); setError(""); try { await api(`/api/admin/students/${student.id}/device/reset`, { method: "POST" }); await load(); } catch (e: any) { setError(e.message); } finally { setBusyId(""); } };
  ...
  return <div className="admin-page"><header className="admin-header"><div><div className="admin-kicker">PromptEx</div><h1>Student Management</h1><p>Welcome, {user.fullName}</p></div><button className="admin-logout" onClick={onLogout}>Logout</button></header>
    ...
    <div className="admin-table-wrap"><table><thead><tr><th>Student</th><th>WhatsApp</th><th>Email</th><th>ID</th><th>Registered</th><th>Status</th><th>Device</th><th>Last login</th><th>Actions</th></tr></thead><tbody>{filtered.map((s) => <tr key={s.id}><td><strong>{s.fullName}</strong></td><td>{s.whatsappNumber}</td><td>{s.email || "—"}</td><td>{s.studentId || "—"}</td><td>{new Date(s.createdAt).toLocaleDateString()}</td><td><span className={`status ${s.status.toLowerCase()}`}>{s.status}</span></td><td>{s.deviceBoundAt ? `Bound ${new Date(s.deviceBoundAt).toLocaleDateString()}` : "Not bound"}</td><td>{s.lastLoginAt ? new Date(s.lastLoginAt).toLocaleString() : "Never"}</td><td><div className="action-row">{s.deviceBoundAt && <button disabled={!!busyId} onClick={() => resetDevice(s)}>Reset Device</button>}{s.status !== "APPROVED" && <button disabled={!!busyId} onClick={() => changeStatus(s, "APPROVED")}>Approve</button>}{s.status === "PENDING" && <button disabled={!!busyId} onClick={() => changeStatus(s, "REJECTED")}>Reject</button>}{s.status === "APPROVED" && <button disabled={!!busyId} onClick={() => changeStatus(s, "SUSPENDED")}>Suspend</button>}{s.status === "SUSPENDED" && <button disabled={!!busyId} onClick={() => changeStatus(s, "APPROVED")}>Reactivate</button>}</div></td></tr>)}</tbody></table>{filtered.length === 0 && <div className="empty">No students in this view.</div>}</div>
  </div>;
}
